...
public class CellularReportTest3 {
	...
	public void test() throws SolutionException, GatewayException, RoutingException, InterruptedException{
		...
		// 1440 reports in 24 hours -  every 1 minute
		ZonedDateTime oneDayAgo = now.minusDays(7);
		System.out.println(oneDayAgo);
		for (int i = 0; i < MINUTES_IN_A_DAY; i += 1) {
			...
			System.out.println(String.format("Submitted cellular report number %d from 1440 for building 1", i + 1));
