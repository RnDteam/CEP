...
public class CellularCallReportTest {
	...
	public void test() throws SolutionException, GatewayException, RoutingException, InterruptedException{
		...
		PersonInitialization criminalCaller = conceptFactory.createPersonInitialization(now);	
		criminalCaller.setPerson(testDriver.createRelationship(Person.class, "criminalCaller"));
		criminalCaller.setName("CriminalCaller caller");
		criminalCaller.setProfession("pro1");
		criminalCaller.setRole(testDriver.createRelationship(OrganizationalRole.class, "role1")); //Criminal
		criminalCaller.setState(PersonState.ACTIVE);
		location = SpatioTemporalService.getService().getGeometryFactory().getPoint( 34.781768 + Math.random(), 32.085300 + Math.random());
		criminalCaller.setLocation(location);
		
		PersonInitialization callee = conceptFactory.createPersonInitialization(now);
		callee.setPerson(testDriver.createRelationship(Person.class, "callee"));
		callee.setName("b2");
		callee.setProfession("pro2");
		callee.setRole(testDriver.createRelationship(OrganizationalRole.class, "role2")); // None Criminal
		callee.setState(PersonState.ACTIVE);
		location = SpatioTemporalService.getService().getGeometryFactory().getPoint( 34.781768 + Math.random(), 32.085300 + Math.random());
		...
		testDriver.submitEvent(criminalCaller);
		testDriver.submitEvent(callee);
